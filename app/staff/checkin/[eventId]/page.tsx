...
export default function StaffCheckInPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const statisticsResponse = await eventService.getEventStatistics(eventId)
        ...
        if (statisticsResponse.success && statisticsResponse.data) {
          ...
          if (data.recentCheckIns && data.recentCheckIns.length > 0) {
            const checkIns: CheckInRecord[] = data.recentCheckIns.map((ci) => ({
              ...
              status: ci.status === 'used' || ci.status === 'checked_in' ? 'entered' : 'already_used',
          ...
          if (event) {
            setStats({
              ...
              checkInRate: event.totalSeats > 0 ? Math.round(((event.registeredCount || 0) / event.totalSeats) * 100) : 0,
            })
          }
        }
      } catch (error: any) {
  ...
  const handleCheckIn = useCallback(async (code?: string) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (message.includes("Already Checked In") || message.includes("đã được sử dụng")) {
          status = 'already_used'
      ...
      if (errorMessage.includes("Already") || errorMessage.includes("đã")) {
        status = 'already_used'
      } else if (errorMessage.includes("Cancelled") || errorMessage.includes("hủy")) {
        status = 'cancelled'
      }
  ...
  }, [ticketCode, handleCheckIn, isProcessing])
      ...
      <div className="space-y-4 max-w-7xl mx-auto p-4 lg:p-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Nhập mã vé hoặc quét QR code để check-in</p>
          </div>
        </div>
          ...
          <CardContent className="p-4">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              ...
              <div className="flex gap-4">
                ...
                <div className="text-center px-4 py-2 bg-background/80 rounded-lg">
                  <p className="text-2xl font-bold text-success">{stats.checkInRate}%</p>
          ...
          <div className="space-y-4">
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-xs">Quét QR code hoặc nhập mã vé thủ công</CardDescription>
              </CardHeader>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base">Trạng thái check-in</CardTitle>
              </CardHeader>
              <CardContent>
                {checkInResult ? (
                  <CheckInStatusCard result={checkInResult} />
                ) : (
                  <div className="text-center py-6 text-muted-foreground text-sm">Nhập mã vé để check-in</div>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <StatusLegendItem status="entered" label="Thành công / Đã vào" />
                <StatusLegendItem status="already_used" label="Đã check-in" />
                <StatusLegendItem status="not_found" label="Không tìm thấy vé" />
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription className="text-xs">Danh sách các lượt check-in mới nhất</CardDescription>
                </div>
            ...
            <CardContent>
              {recentCheckIns.length === 0 ? (
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="text-xs">TÊN NGƯỜI THAM GIA</TableHead>
                  ...
                  <TableBody>
                    {recentCheckIns.map((record, index) => (
                      <TableRow
                        key={record.id}
                        className={index === 0 && checkInResult?.status === "entered" ? "bg-success/5" : ""}
                      >
    ...
    <div className={`p-4 rounded-lg border ${config.bgColor} text-center`}>
      ...
      {result.attendeeName && (
        <p className="text-sm text-muted-foreground mt-1">
          {result.status === "entered" ? `Chào mừng, ${result.attendeeName}!` : `${result.attendeeName} - ${result.time}`}
        </p>
      )}
      {result.seatInfo && <p className="text-sm font-medium mt-1">Ghế: {result.seatInfo}</p>}
      {result.status === "not_found" && (
        <p className="text-sm text-muted-foreground mt-1">Mã &apos;{result.ticketCode}&apos; không tồn tại</p>
      )}
    </div>
  )
}
...
function StatusBadge({ status }: { status: CheckInRecord['status'] }) {
  const config = {
    ...
    already_used: { label: "Đã sử dụng", className: "bg-warning/10 text-warning" },
    not_found: { label: "Không tìm thấy", className: "bg-destructive/10 text-destructive" },
...
// Status Legend Item
function StatusLegendItem({ status, label }: { status: CheckInResult['status']; label: string }) {
  const config = {
